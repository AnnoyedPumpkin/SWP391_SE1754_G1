...
public class AuthentController extends HttpServlet {
    ...
    private void register(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!isExist) {
            ...
            boolean isInserted = commonDAO.register(account);
            if (isInserted) {
    ...
    private void changepass(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // Kiểm tra xem mật khẩu moi và mật khẩu moi nhập lại có giống nhau không
        if (!newPassword.equals(newPassword2)) {
            request.setAttribute("error1", "Mật khẩu moi phải giống nhau");
            request.getRequestDispatcher("changepass.jsp").forward(request, response);
            return; // Kết thúc phương thức để không thực hiện các bước tiếp theo nếu mật khẩu không khớp
        }
        Account account = Account.builder()
